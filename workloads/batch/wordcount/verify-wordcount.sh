...
# Running command: ./verify-wordcount.sh <volume> <run> <system>
...
a=$1
hdfs_directory=/outputs/batch/wordcount/wordcount-$a"GB"-results
unicage_directory=~/datav/outputs/batch/wordcount/wordcount-$a"GB"-results
local_directory=~/datav/outputs/batch/wordcount/wordcount-$a"GB"-results/$2
benchmark_directory=~/implementation/benchmarks/benchmark-results/batch/wordcount/wordcount-$a"GB"/
big_data_system=$3
...
[ $big_data_system == hadoop ] && {
    ...
    cat $local_directory/hadoop-result | sed s/'\t'/','/g | sort -k1,1 > $local_directory/hadoop-result.tmp
    mv $local_directory/hadoop-result.tmp $local_directory/hadoop-result
...
[ $big_data_system == spark ] && {
    ...
    cat $local_directory/spark-result | sed 's/[()]//g' | sort -k1,1 > $local_directory/spark-result.tmp
...
[ $big_data_system == unicage ] && {
    ...
    cat $local_directory/unicage-result | tr " " "," | sort -k1,1 > $local_directory/unicage-result.tmp
