...
# Running command: ./genData-sort_cluster.sh <volume>
...
# Reformatting the input data for the sort workload
...
for i in $producers; do
    ...
    ping -c 1 $i &> /dev/null && {
        ...
        ssh $i 'cd ~/datav/datasets/'$directory'

        count=1

        while [ -f "lda_wiki1w_"$count"_"${HOSTNAME} ]; do
    
            echo "Formatting lda_wiki1w_"$count"_"${HOSTNAME}
            tr -s '"'"'[:blank:]'"'"' '"'"'\n'"'"' <lda_wiki1w_${count}_${HOSTNAME} >lda_wiki1w_${count}_${HOSTNAME}_tmp
            mv lda_wiki1w_${count}_${HOSTNAME}_tmp lda_wiki1w_${count}_${HOSTNAME}
            count=$(( $count + 1 )) 
