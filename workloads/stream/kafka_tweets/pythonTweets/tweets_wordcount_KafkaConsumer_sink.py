...
query = table.selectExpr("CAST(value AS STRING)", "CAST(key AS STRING)").writeStream.format("kafka").option("kafka.bootstrap.servers", "localhost:9092").option("topic", "tweets-output").option("checkpointLocation", save_file + "/checkpoints/KafkaConsumersink").start()
