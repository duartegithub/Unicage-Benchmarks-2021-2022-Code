...
# (the repartition option creates one file per distinct 'window' column, so one file per timestamp)
query = table.selectExpr("CAST(value AS STRING)", "CAST(key AS STRING)").repartition(1, col("window")).writeStream.format("json").option("path", save_file).option("truncate","false").outputMode("append").option("checkpointLocation", save_file + "/checkpoints/HDFSsink").start()
